...
public class PrioriBestPrHelperACHPWY {
  static ACHPWYSketchSimuCompact simuWorker=null,simuWorkerSmall1=null,simuWorkerSmall2=null;
  static int maxMemoryNum;
  static int[] initialCompactNum;
  static int MERGE_BUFFER_RATIO=0,MULTI_QUANTILE=1;

  public PrioriBestPrHelperACHPWY(int maxMemoryNum, int queryN, int multi_quantile){
    PrioriBestPrHelperACHPWY.maxMemoryNum=maxMemoryNum;
    simuWorker = new ACHPWYSketchSimuCompact(maxMemoryNum);
    initialCompactNum = simuWorker.simulateCompactNumGivenN(queryN);
    MULTI_QUANTILE = multi_quantile;
    simuWorkerSmall1=new ACHPWYSketchSimuCompact(maxMemoryNum/MULTI_QUANTILE);
    simuWorkerSmall2=new ACHPWYSketchSimuCompact(maxMemoryNum/MULTI_QUANTILE);
  }

  public PrioriBestPrHelperACHPWY(int maxMemoryNum, int queryN, int []firstPassCompactNum, int merge_buffer_ratio, int multi_quantile){
    PrioriBestPrHelperACHPWY.maxMemoryNum=maxMemoryNum;
    simuWorker = merge_buffer_ratio==0?new ACHPWYSketchSimuCompact(maxMemoryNum):
        new ACHPWYSketchSimuCompact(maxMemoryNum*(merge_buffer_ratio-1)/merge_buffer_ratio);
  ...
  public PrioriBestPrHelperACHPWY(int maxMemoryNum, int queryN, int merge_buffer_ratio, int multi_quantile){
    PrioriBestPrHelperACHPWY.maxMemoryNum=maxMemoryNum;
    simuWorker = merge_buffer_ratio==0?new ACHPWYSketchSimuCompact(maxMemoryNum):
        new ACHPWYSketchSimuCompact(maxMemoryNum*(merge_buffer_ratio-1)/merge_buffer_ratio);
    initialCompactNum = simuWorker.simulateCompactNumGivenN(queryN);
...
//      System.out.println("\t\t\tp1:"+(1-err1)+"\tV1:"+evaluateP1+"\t\t\tp2:"+(1-err2)+"\tV2:"+evaluateP2);
    ...
    double[] result = (p1Better) ? new double[]{1 - err1, evaluateP1}
        : new double[]{1 - err2, evaluateP2};
  ...
  private static ACHPWYSketchSimuCompact findASimulator(int n){
//    return simuWorkerSmall1; // similar efficiency...
    if(simuWorkerSmall1.cntN<simuWorkerSmall2.cntN)
      return (n>= simuWorkerSmall1.cntN&&n< simuWorkerSmall2.cntN)?simuWorkerSmall1:simuWorkerSmall2;
    else
      return (n>= simuWorkerSmall2.cntN&&n< simuWorkerSmall1.cntN)?simuWorkerSmall2:simuWorkerSmall1;
  }


  private static double simulateIteration(double casePr, double fixPr, int depth, int maxMemoryNum,int avgN,double sig2,long maxERR) {
...
//        System.out.println("\t\t\t\t?????\t\tavgN:"+avgN+"\t\tmaxMem:"+maxMemoryNum+"\t\tfinishPr:"+finishPr);
    ...
    TruncatedNormalDistribution tnd = TruncatedNormalDistribution.of(avgN, Math.sqrt(sig2), memoryNumForEachQuantile, Double.MAX_VALUE);
...
//    }
    ...
    double conSig2 = ACHPWYSketchSimuCompact.getSig2(conCompactNum);
    long conMaxErr = ACHPWYSketchSimuCompact.getMaxError(conCompactNum);
...
//        System.out.println("\tdepth:"+depth+" avgN:"+avgN+"\tfixPr:"+fixPr+"\t\tcontinuePr:"+continuePr+"\t\t\tconAvgN:"+continueAvgN+"\t\tconMaxERR:"+conMaxErr);
    ...
    int conSuccessN = conMaxErr==0?1:KLLSketchLazyExactPriori.queryRankErrBoundGivenParameter(conSig2, conMaxErr, allSuccessPr) * 2;
...
//        bestSimuIter = 1 + pr * simulateIteration(casePr * pr, pr, depth + 1, successN, maxMemoryNum, succComNum)[0] + (1 - pr) * (1 + simulateIteration(casePr * (1 - pr), pr, depth + 1, failN, maxMemoryNum, failComNum)[0]);
    double simuPrF =
        allFinishPr * 1.0 +
            continuePr * (
                1.0 + fixPr * simulateIteration(casePr * fixPr, fixPr, depth + 1, maxMemoryNum, conSuccessN, conSig2, conMaxErr)
                    + (1 - fixPr) * (1.0 + simulateIteration(casePr * (1 - fixPr), fixPr, depth + 1, maxMemoryNum, conFailN, conSig2, conMaxErr))
            );
    return simuPrF;
  }

  public static double evaluatePrFromScratch(int maxMemoryNum, double fixPr, int detN, int merge_buffer_ratio,int multi_quantile) {
    int firstPassMemory = merge_buffer_ratio==0?maxMemoryNum:maxMemoryNum*(merge_buffer_ratio-1)/merge_buffer_ratio;
...
//        if(DEBUG_PRINT)System.out.println("\t\t\t\t\t\t\t\tcntPR:"+fixPr+"\tsuccessN:\t"+succN+"\t\tfailN:\t"+failN+/*"\t\testi_iter:\t"+estimateIterationNum+*/"\t\tsimu_iter:\t"+simulateResult[0]+"\tsimu_nextSuccessPr:"+simulateResult[1]);
  ...
  private static double evaluatePr(int maxMemoryNum, double fixPr, int detN) {
    ...
    double sig2 = ACHPWYSketchSimuCompact.getSig2(initialCompactNum);
    long nextMaxErr= ACHPWYSketchSimuCompact.getMaxError(initialCompactNum);
    double allSuccessPr = MULTI_QUANTILE==1?fixPr:Math.pow(fixPr,1.0/MULTI_QUANTILE);
...
//        System.out.println("\t\tfirst paas. successAvgN:\t"+successAvgN+"\t\tfailAvgN:\t"+failAvgN+"\t\tfixPr:"+fixPr);
//        System.out.println("\t\t\t\t\t"+sig2+"\t"+maxERR+"\t\t\tnextMaxERR:\t"+nextMaxErr);
