...
//      System.out.println("\t\t2fen\t\t"+L+"..."+R+"\t\tmid="+mid+"\t\t"+(getApproxRank(mid)>=K));
  ...
  public void findMinValueWithRankMulti(LongArrayList Ks, LongArrayList Vs,int pL,int pR,long vL,long vR){
    if(vL>=vR){
      for(int i=pL;i<pR;i++)Vs.add(vL);
      return;
    }
    long vMid=vL + ((vR - vL) >>>1);
    int rankMid = getApproxRank(vMid);
    int pMid=pL;
    while(pMid<pR&&rankMid>Ks.getLong(pMid))pMid++;
    if(pL<pMid)findMinValueWithRankMulti(Ks,Vs,pL,pMid,vL,vMid);
    if(pMid<pR)findMinValueWithRankMulti(Ks,Vs,pMid,pR,vMid+1,vR);
  }
  public LongArrayList findMinValueWithRankMulti(LongArrayList Ks,long minV,long maxV){
    LongArrayList Vs = new LongArrayList();
    findMinValueWithRankMulti(Ks,Vs,0,Ks.size(),Long.MIN_VALUE,Long.MAX_VALUE);
    return Vs;
  }
